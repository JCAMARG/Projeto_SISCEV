...
CREATE OR REPLACE PACKAGE BODY PKG_COMPRAS AS
	...
	--> PROCEDURE 01 - Criar Pedido de Compra (Cabeçalho)
	PROCEDURE Criar_Pedido_Compra (
		p_id_pedido     IN VARCHAR2,
		p_id_fornecedor IN VARCHAR2,
		p_emissao       IN DATE,
	...
	END Criar_Pedido_Compra;

	--> PROCEDURE 02 - Inserir Item no Pedido de Compra
	PROCEDURE Inserir_Item_Compra (
		p_id_item     IN VARCHAR2,
		p_id_pedido   IN VARCHAR2,
		p_id_produto  IN VARCHAR2,
...
CREATE OR REPLACE PACKAGE BODY PKG_VENDAS AS
	...
	--> PROCEDURE 03 - Criar Pedido de Venda (Cabeçalho)
    PROCEDURE Criar_Pedido_Venda (
        p_id_venda     IN VARCHAR2,
        p_id_cliente   IN VARCHAR2,
        p_id_vendedor  IN VARCHAR2,
    ...
    END Criar_Pedido_Venda;

	--> PROCEDURE 04 - Inserir Item no Pedido de Venda
    PROCEDURE Inserir_Item_Venda (
        p_id_item     IN VARCHAR2,
        p_id_venda    IN VARCHAR2,
        p_id_produto  IN VARCHAR2,
...
CREATE OR REPLACE PACKAGE PKG_FINANCEIRO AS
    ...
    PROCEDURE Gerar_Titulos_Receber (
...
CREATE OR REPLACE PACKAGE BODY PKG_FINANCEIRO AS
	...
	--> PROCEDURE 05 - Gerar Títulos a Pagar (NFE)
    PROCEDURE Gerar_Titulos_Pagar (
        p_id_nfe      IN VARCHAR2,
        p_parcelas    IN NUMBER,
        p_valor_total IN NUMBER,
    ...
    BEGIN
        ...
        END LOOP;
    END Gerar_Titulos_Pagar;

	--> PROCEDURE 06 - Gerar Títulos a Receber (NFS)
    PROCEDURE Gerar_Titulos_Receber (
        p_id_nfs      IN VARCHAR2,
        p_parcelas    IN NUMBER,
        p_valor_total IN NUMBER,
